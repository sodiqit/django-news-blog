...
class CategoryConverter:
    def convert_categories_to_tree(self, categories: list[Category]):
        grouped_categories: list[list[Category]] = reduce(self._group_by_id, categories, [])
        data = []

        for group in grouped_categories:
            group.sort(key=lambda category: category.id)
            data.append(self._group_by_parent_category(group))

        return data

    def _group_by_id(self, prev: list, current: Category):
        """
    ...
    def _find_related_item(self, parent_id: int):
        """
    ...
    def _group_by_parent_category(self, group: list[Category], item: Category | None = None):
        """
        ...
        parent_id = item.id
        related_items = list(filter(self._find_related_item(parent_id), group))
        ...
        return {
            "id": item.category_id,
            "name": item.title,
            "items": [self._group_by_parent_category(group, x) for x in related_items],
        }
